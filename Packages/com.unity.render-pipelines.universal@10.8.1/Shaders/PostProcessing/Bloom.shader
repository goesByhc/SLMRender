Shader "Hidden/Universal Render Pipeline/Bloom"
{
    HLSLINCLUDE
        ...
        half4 FragPrefilter(Varyings input) : SV_Target
        {
            ...
            //Custom Start
            //这里取出A通道的值，乘以一个系数，来实现Bloom的范围取值，下同
            half3 color = o.xyz;
            color = color.rgb * (1 - o.a) * 10;
        #else
            // half3 color = SAMPLE_TEXTURE2D_X(_SourceTex, sampler_LinearClamp, uv).xyz;
            half4 color4 = SAMPLE_TEXTURE2D_X(_SourceTex, sampler_LinearClamp, uv);
            half3 color = color4.rgb * (1 - color4.a) * 10;
            //Custom End
        #endif
