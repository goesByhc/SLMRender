...
namespace UnityEditor.Rendering.Universal
{
    ...
    internal class ShaderPreprocessor : IPreprocessShaders
    {
        ...
        int StripUnused(ShaderFeatures features, Shader shader, ShaderSnippetData snippetData, ShaderCompilerData compilerData)
        {
        ...
        void LogShaderVariants(Shader shader, ShaderSnippetData snippetData, ShaderVariantLogLevel logLevel, int prevVariantsCount, IList<ShaderCompilerData> currVariants)
        {
            if (logLevel == ShaderVariantLogLevel.AllShaders || shader.name.Contains("Universal Render Pipeline"))
            {
                var datas = currVariants as List<ShaderCompilerData>;
                
                float percentageCurrent = (float)datas.Count / (float)prevVariantsCount * 100f;
                float percentageTotal = (float)m_TotalVariantsOutputCount / (float)m_TotalVariantsInputCount * 100f;

                string result = string.Format("STRIPPING: {0} ({1} pass) ({2}) -" +
                        " Remaining shader variants = {3}/{4} = {5}% - Total = {6}/{7} = {8}%",
                        shader.name, snippetData.passName, snippetData.shaderType.ToString(), currVariants.Count,
                ...
                foreach (var data in datas)
                {
                    ...
                    {
                        s.Append(ShaderKeyword.GetKeywordName(shader, keyword) + " ");
                    }
    
                    Debug.Log($"  --- Include Shader No.{++index}, <{shader.name}> variants, passType:{snippetData.passType}, passName:{snippetData.passName}, Keywords: {s}");
                }
            }
        }
        ...
        public void OnProcessShader(Shader shader, ShaderSnippetData snippetData, IList<ShaderCompilerData> compilerDataList)
        {
            ...
            var dataList = compilerDataList as List<ShaderCompilerData>;
            
            int prevVariantCount = compilerDataList.Count;
            var inputShaderVariantCount = compilerDataList.Count;

            if (shader.name == "Scene_Vegetation")
            {
                for (int i = 0; i < inputShaderVariantCount; i++)
                {
                    var keywords = dataList[i].shaderKeywordSet.GetShaderKeywords();
                    var s = new StringBuilder();
                    foreach (var keyword in keywords)
                    {
                        s.Append(ShaderKeyword.GetKeywordName(shader, keyword) + " ");
                    ...
                    Debug.Log($"Shader <{shader.name}> variatn Keywords: {s}");
                }
            }
            
            for (int i = 0; i < inputShaderVariantCount;)
            {
                var removeInput = StripUnused(ShaderBuildPreprocessor.supportedFeatures, shader, snippetData, compilerDataList[i]);
                if (removeInput > 0)
                {
                    if (urpAsset.shaderVariantLogLevel == ShaderVariantLogLevel.AllShaders)
                    {
                        var alphaOn = dataList[i].shaderKeywordSet.IsEnabled(m_AlphaTestOn);
                        var lightMapOn = dataList[i].shaderKeywordSet.IsEnabled(m_Lightmap);
                        if (alphaOn && lightMapOn)
                        {
                            var keywords = dataList[i].shaderKeywordSet.GetShaderKeywords();
                            var s = new StringBuilder();
                            foreach (var keyword in keywords)
                            ...
                            Debug.Log($"[Strip Unused] {removeInput} passType: {snippetData.passType}, passsName:{snippetData.passName}, variatn Keywords : {s}");
                        }
                    }
            ...
            if (urpAsset.shaderVariantLogLevel != ShaderVariantLogLevel.Disabled)
            {
                ...
                LogShaderVariants(shader, snippetData, urpAsset.shaderVariantLogLevel, prevVariantCount, compilerDataList);
            }
            m_stripTimer.Stop();
