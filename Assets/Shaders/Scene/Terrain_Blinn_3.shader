Shader "Terrain_Blinn_3"
{
    Properties
    {
        ...
        [KeywordEnum(R,G,B,A)] _Blend_ChannelA("Blend Channel A", Int) = 0
        [KeywordEnum(R,G,B,A)] _Blend_ChannelB("Blend Channel B", Int) = 1
        [KeywordEnum(R,G,B,A)] _Blend_ChannelC("Blend Channel C", Int) = 1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #pragma shader_feature_local _BLEND_CHANNELA_R _BLEND_CHANNELA_G _BLEND_CHANNELA_B _BLEND_CHANNELA_A
            #pragma shader_feature_local _BLEND_CHANNELB_R _BLEND_CHANNELB_G _BLEND_CHANNELB_B _BLEND_CHANNELB_A
            #pragma shader_feature_local _BLEND_CHANNELC_R _BLEND_CHANNELC_G _BLEND_CHANNELC_B _BLEND_CHANNELC_A
            ...
            half Blend(half4 blendFactor, half p1, half p2, half p3)
            {
                
                #if defined(_BLEND_CHANNELA_R)
                half channalA = blendFactor.r;
                #elif defined(_BLEND_CHANNELA_G)
                half channalA = blendFactor.g;
                #elif defined(_BLEND_CHANNELA_B)
                half channalA = blendFactor.b;
                #elif defined(_BLEND_CHANNELA_A)
                half channalA = blendFactor.a;
                #else
                half channalA = 1;
                #endif
                
                #if defined(_BLEND_CHANNELB_R)
                half channalB = blendFactor.r;
                #elif defined(_BLEND_CHANNELB_G)
                half channalB = blendFactor.g;
                #elif defined(_BLEND_CHANNELB_B)
                half channalB = blendFactor.b;
                #elif defined(_BLEND_CHANNELB_A)
                half channalB = blendFactor.a;
                #else
                half channalB = 1;
                #endif


                #if defined(_BLEND_CHANNELC_R)
                half channalC = blendFactor.r;
                #elif defined(_BLEND_CHANNELC_G)
                half channalC = blendFactor.g;
                #elif defined(_BLEND_CHANNELC_B)
                half channalC = blendFactor.b;
                #elif defined(_BLEND_CHANNELC_A)
                half channalC = blendFactor.a;
                #else
                half channalC = 1;
                #endif
                
                
                return channalA * p1 + channalB * p2 + channalC * p3;
            }

            half4 Blend(half4 blendFactor, half4 p1, half4 p2, half p3)
            {
            ...
            half4 frag (VertexOutputObjectBlinn i) : SV_Target
            {
                ...
                //Blend
                half3 baseColor = Blend(blendMap, layer1, layer2, layer3).rgb;
                half4 _Color = Blend(blendMap, _Layer1_Color, _Layer2_Color, _Layer3_Color);
                half _Brightness = Blend(blendMap, _Layer1_Brightness, _Layer2_Brightness, _Layer3_Brightness);
                half _Saturation = Blend(blendMap, _Layer1_Saturation, _Layer2_Saturation, _Layer3_Saturation);
                half4 _SpecColor = Blend(blendMap, _Layer1_SpecColor, _Layer2_SpecColor, _Layer3_SpecColor);
                half _Roughness = Blend(blendMap, _Layer1_Roughness, _Layer2_Roughness, _Layer3_Roughness);
                half _SpecIntensity = Blend(blendMap, _Layer1_SpecIntensity, _Layer2_SpecIntensity, _Layer3_SpecIntensity);
                half4 _RimColor = Blend(blendMap, _Layer1_RimColor, _Layer2_RimColor, _Layer3_RimColor);
                half4 _RimPower = Blend(blendMap, _Layer1_RimPower, _Layer2_RimPower, _Layer3_RimPower);
                half4 _RimIntensity = Blend(blendMap, _Layer1_RimIntensity, _Layer2_RimIntensity, _Layer3_RimIntensity);
